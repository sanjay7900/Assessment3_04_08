using Assessment3_04_08_BoolSale.Models;
using Microsoft.AspNetCore.Mvc;
using Newtonsoft.Json;

namespace Assessment3_04_08_BoolSale.Controllers
{
    public class BooksController : Controller
    {
        ...
        public ActionResult AddToCart(int id)
        {
            try
            {
                ...
                Book? book = JsonConvert.DeserializeObject<Book>(stringData);
                var cart = new cart()
                {
                    Name = book!.Name,
                    Cost = book.Cost,
        ...
        {
            HttpResponseMessage response = httpClient.DeleteAsync(baseAddress + "Cart/Delete?id=" + id).Result;
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction(nameof(Viewcart));
            }
